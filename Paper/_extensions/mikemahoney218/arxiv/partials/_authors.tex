...
$-- %%%%%%%%%%%%%%%
\textbf{$it.name.literal$}$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$for(it.affiliations)$\\$it.department$\\$it.name$\\$if(it.city)$$it.city$ $if(it.country)$ ($it.country$)$if(it.postal-code)$,\ $it.postal-code$$endif$$endif$$endif$$endfor$\\$if(it.email)$\href{mailto:$it.email$}{$it.email$}$endif$
